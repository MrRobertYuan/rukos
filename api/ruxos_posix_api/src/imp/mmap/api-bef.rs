...
pub fn sys_mmap(
    ...
    syscall_body!(sys_mmap, {
        ...
        // check if `start` is aligned to `PAGE_SIZE_4K`
        ...
        match try_addr {
            Some(vaddr) => {
                ...
                warn!("mmap addr: 0x{:x}, len: 0x{:x}", vaddr, len);
...
pub fn sys_msync(start: *mut c_void, len: ctypes::size_t, flags: c_int) -> c_int {
    ...
    syscall_body!(sys_msync, {
        #[cfg(feature = "fs")]
        {
            ...
            let mut cleaned_pages: Vec<usize> = Vec::new();
            for (&vaddr, page_info) in memory_map.range(start..end) {
                if let Some((file, offset, size)) = page_info {
                    ...
                    write_into(file, src, *offset as u64, *size);
                     // recycle the clean page.
                     cleaned_pages.push(vaddr);
                    }
    
                }
                for vaddr in cleaned_pages {
